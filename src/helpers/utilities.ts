...
export function getPriceLabel(order: Order) {
  ...
  const priceLabel = parseFloat(price.toString()).toLocaleString(undefined, {
    minimumSignificantDigits: 1,
  });
  const isETH = paymentTokenContract.symbol === 'ETH';
  return `${isETH ? 'Ξ' : ''} ${priceLabel} ${
    isETH ? '' : paymentTokenContract.symbol
  }`;
}

export const truncate = (input, characterLength) =>
  input.length > characterLength
    ? `${input.substring(0, characterLength)}...`
    : input;

export const getOSNetwork = () =>
  process.env.OPEN_SEA_MAINNET || process.env.NEXT_PUBLIC_OPEN_SEA_MAINNET
    ? Network.Main
    : Network.Rinkeby;
